...
contract Payment {
    ...
    function authorize(string memory _merchantId, string memory _transactionId, uint256 _amount) public view returns (string memory response) {
        string id = this.newId(_transactionId);        
        Transaction trx = Transaction(_amount, id, _merchantId, _from, _to);
        auths[id] = trx;
        return trx;
