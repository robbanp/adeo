...
contract Payment {
    ...
    function authorize(string memory _merchantId, string memory _transactionId, uint256 memory _amount, address memory _receiver) public view returns (string memory response) {
        string id = this.newId(_transactionId);        
        Transaction trx = Transaction(_amount, id, _merchantId, msg.sender, _receiver);
        auths[id] = trx;
        return trx;
